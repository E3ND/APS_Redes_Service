...
public interface ChatGroupReposittory extends JpaRepository<ChatGroup, Long> {
	...
	@Query(nativeQuery = true, value = "SELECT chatUser.id AS 'chatUserId', user.id AS 'userId', user.name AS 'name', user.email AS 'email', user.image_name AS 'userImageName' FROM railway.chat_user chatUser "
