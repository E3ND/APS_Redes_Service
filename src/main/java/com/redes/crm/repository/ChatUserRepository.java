...
public interface ChatUserRepository extends JpaRepository<ChatUser, Long> {
	...
	@Query(nativeQuery = true, value = "SELECT chatUser.user_id as 'userId' FROM railway.chat_user chatUser "
			+ "INNER JOIN railway.conversation conversation ON chatUser.conversation_id = conversation.id "
	...
	@Query(nativeQuery = true, value = "SELECT chatGroup.id AS 'chatGroupId' ,chatGroup.conversation_id AS 'conversationId', chatUser.user_id AS 'userId' FROM railway.chat_user chatUser "
			+ "INNER JOIN railway.chat_group chatGroup ON chatGroup.conversation_id = chatUser.conversation_id "
	...
	@Query(nativeQuery = true, value = "SELECT * FROM railway.chat_user chatUser "
	...
	@Query(nativeQuery = true, value = "SELECT conversation.id AS 'conversationId', conversation.created_at AS 'conversationCreatedAt' FROM railway.chat_user AS chatUser1 "
			+ "INNER JOIN railway.chat_user AS chatUser2 ON chatUser1.conversation_id = chatUser2.conversation_id "
			+ "INNER JOIN railway.conversation AS conversation ON conversation.id = chatUser1.conversation_id "
			+ "WHERE chatUser1.user_id = :userId "
			+ "AND chatUser2.user_id = :otherUserId "
			+ "AND NOT EXISTS ( "
			+ "    SELECT 1 "
			+ "    FROM railway.chat_user AS otherUser "
			+ "    WHERE otherUser.conversation_id = chatUser1.conversation_id "
			+ "    AND otherUser.user_id NOT IN (:userId, :otherUserId) "
			+ ") "
			+ "AND NOT EXISTS ( "
			+ "    SELECT 1 "
			+ "    FROM railway.chat_group AS chatGroup "
